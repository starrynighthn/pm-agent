...
[![Version](https://img.shields.io/badge/version-v1.0.1-blue.svg)](https://github.com/starrynighthn/pm-agent)
...
## 🧑‍💻 Hướng dẫn Setup Cá nhân cho Thành viên Mới (Newcomer Setup Guide)

Để bắt đầu làm việc với dự án mà không vô tình đưa các thông tin cá nhân (như đường dẫn tuyệt đối cục bộ, tài khoản Git cá nhân, hoặc URL Jira mạng nội bộ riêng tư) vào kho lưu trữ chung (Repository), hãy tuân thủ hướng dẫn thiết lập sau:

### 1. Không dùng đường dẫn tuyệt đối (Absolute Paths)
* **Luôn sử dụng đường dẫn tương đối (Relative Paths)** bắt đầu từ thư mục gốc của dự án (ví dụ: `rules/project-rules.md` thay vì `/Users/username/Documents/projects/pm-agent/rules/project-rules.md`).
* Mọi script tự động hóa trong `scripts/` đã được thiết lập để tự động xác định thư mục gốc của dự án một cách động. Bạn không cần sửa đổi đường dẫn trong script.

### 2. Định cấu hình Jira URL & Domain nội bộ
* Mặc định, bộ khung sử dụng URL giả lập: `https://jira.company.com/browse/`.
* Mỗi cá nhân hoặc dự án khi triển khai cần thay thế URL này bằng URL Jira chính thức của đơn vị trong file `rules/validation-rules.md` và `rules/project-rules.md`.
* **Lưu ý:** Không commit URL chứa địa chỉ IP mạng nội bộ riêng tư (ví dụ `10.x.x.x` hoặc `192.168.x.x`) lên các repository Git công khai.

### 3. Tận dụng tệp loại trừ của Git (`.gitignore`)
* Hãy chắc chắn các file rác phát sinh trong quá trình chạy của AI Agent hoặc cấu hình cục bộ của IDE (`.vscode/`, `.idea/`, `.gemini/`, `.deepseek/`, v.v.) luôn nằm trong file `.gitignore` để tránh bị commit nhầm lên remote branch.
...
- **Changelog:**
  - `v1.0.1`: Cập nhật bộ quy tắc (rule set) sang tiếng Anh thống nhất, bổ sung hướng dẫn thiết lập cá nhân cho thành viên mới (Newcomer Setup Guide), loại bỏ và làm sạch các liên kết private/địa chỉ IP nội bộ, và cấu trúc hóa đồng bộ các quy tắc.
